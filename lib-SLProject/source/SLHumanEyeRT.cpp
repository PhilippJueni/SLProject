...
*/
SLbool SLHumanEyeRT::renderDistrib(SLSceneView* sv)
{
    _sv = sv;
    _state = rtBusy;                    // From here we state the RT as busy
    _stateGL = SLGLState::getInstance();// OpenGL state shortcut
    ...
    auto sampleAAPixelsFunction = bind(&SLHumanEyeRT::sampleAAPixels, this, _1);
    auto renderSlicesFunction   = _cam->lensSamples()->samples() == 1 ? 
                                  bind(&SLHumanEyeRT::renderSlices, this, _1) : 
                                  bind(&SLHumanEyeRT::renderSlicesMS, this, _1);
...
*/
void SLHumanEyeRT::renderSlices(const bool isMainThread)
{
    // Time points
    double t1 = 0;
...
main thread is allowed to call a repaint of the image.
*/
void SLHumanEyeRT::renderSlicesMS(const bool isMainThread)
{
    // Time points
    double t1 = 0;
...
appoximation.
*/
SLCol4f SLHumanEyeRT::traceDistrib(SLRay* ray)
{  
    SLScene* s = SLScene::current;
    SLCol4f color(s->backColor());
...
//-----------------------------------------------------------------------------
/*!
SLHumanEyeRT::sampleAAPixels does the subsampling of the pixels that need to be
...
main thread is allowed to call a repaint of the image.
*/
void SLHumanEyeRT::sampleAAPixels(const bool isMainThread)
{  
